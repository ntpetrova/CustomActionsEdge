...
(function () {
    ...
    function googleSearchImprovements() {
        ...
        if (!ggHelper) {
            ...
            if (bodyTag) {
                ...
                var keywordList = document.querySelectorAll('#ggHelper .gg-keyword');
                for(var i = 0; i < keywordList.length; i++) {
                    keywordList[i].addEventListener('click', ggReplaceAndSearch);
                };


                var ranges = document.querySelectorAll('#ggHelper .gg-range');
        for(var i = 0; i < ranges.length; i++) {
                    ranges[i].addEventListener('click', ggSetTimeRange);
                };
            }
        }
    }
    ...
    function forEachList(nodes, callback)
    {
        if(!nodes)
            return;
        ...
        function sanitizeParent(el) {
            if (el.classList && el.classList.contains('post_teaser')) {
                forEachList(el.querySelectorAll('img'), function (img) { img.style.display = 'none'; });
                forEachList(el.querySelectorAll('.post__body_crop'), function (chld) {
                    chld.style.maxHeight = '4em';
                    chld.style.overflow = 'hidden';

                    el.addEventListener('mouseover', function () {
                        chld.style.maxHeight = "inherit"; 
                        forEachList(chld.querySelectorAll('img'), function (img) {
                ...
                forEachList(el.querySelectorAll('.post__title a'), function (titl) { titl.style.color = '#707040'; });

            } else if (el.parentElement)
                sanitizeParent(el.parentElement);
        }

        forEachList(document.querySelectorAll('a[href*="https://geektimes.ru/hub/"]'), function (el) {
            var hub = el.getAttribute('href').replace(/^.*\.ru\/hub\//, '').replace(/\/.*$/, '');
            if (data && data.hideHubs && data.hideHubs.indexOf(hub) >= 0)
                hideParent(el);
        });

        forEachList(document.querySelectorAll('a[href*="https://geektimes.ru/company/"], a[href*="https://habrahabr.ru/company/"]'), function (el) {
    ...
    var demoItems = [
        {
            ...
            url: '^(https?://www.google.[a-z]{2,3}(/$|/.gws_rd=ssl$|/web|/search|/#)|chrome://newtab)',
   ...
   angular.module('customActions', []).controller('options', ['$scope', function($scope) {
        $scope.vm = {
                   items: [ newItem() ],
                   status: '',
                   templates: availableTemplates,
                   outputOptions: ['Owner tab', 'Active tab'],
                   dataTypes: ['None', 'Text', 'JSON', 'Html', 'CSS', 'Xml', 'XPath'],
                   executeOptions: ['Page loaded', 'On before load', 'Page loading', 'Popup menu', 'Context menu', 'Delayed 1 sec.', 'Delayed 5 sec.', 'Short-cut (Ctrl-B)', 'Short-cut (Ctrl-Q)', 'Short-cut (Ctrl-Y)']
        };

        function newItem() {
            var item = {
                id: 'act-' + parseInt(new Date().getTime() / 1000),
                name: '',
                url: '',
                sourceType: 'JS',
                source: '',
        ...
        $scope.save = function () {
            var items = { actions: JSON.parse(angular.toJson($scope.vm.items)) };

            browser.storage.local.set(items, function () {
                $scope.vm.status = 'Items saved.';
                setTimeout(function () { $scope.$apply(); }, 0);
                setTimeout(function () { $scope.vm.status = ''; $scope.$apply(); }, 750);
        ...
        $scope.delete = function (item) {
            if (confirm('Are you sure you want to delete the action?')) {            
                var idx = $scope.vm.items.indexOf(item);
                $scope.vm.items.splice(idx, 1);
            }
        };
        ...
        $scope.toggleSource = function (data, event) {
            var pre = event.target.parentElement.querySelector('pre');
            pre.style.display = 'block';
        }

        $scope.getTemplateSource = function (item) {
                return stripFunctionBody(getTemplate(item.sourceType));
        }
   ...
   document.addEventListener('DOMContentLoaded', function() {
       angular.bootstrap(document, ['customActions']);
   });  
